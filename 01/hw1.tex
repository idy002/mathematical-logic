...
\begin{document}
	...
	\begin{sol}{2.3}
		...
		Assume $f$ is a surjective function from $M$ to $Pow(M)$.

		We construct a set $S$ such that $x \in S$ if and only if $x \not \in f(x)$.
		
		There does not exist an element $x$ in $M$ such that $f(x) = S$. So $f$ is not surjective. 
		
		Thus, there is not a surjective function from $M$ to $Pow(M)$. 
	\end{sol}
